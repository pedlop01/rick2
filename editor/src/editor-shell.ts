...
export function createEditorShell(host: HTMLElement): void {
  ...
  function renderProfileInspector(): void {
    ...
    inspector.append(details);
    const combatDetails = document.createElement("details"); combatDetails.className = "profile-section"; combatDetails.open = true; combatDetails.append(Object.assign(document.createElement("summary"), { textContent: "Combat profiles" }));
    const level = levelModel.level as Record<string, unknown>, combat = level.combat as { damageTypes: Array<{ id: string }>; profiles: Array<Record<string, unknown>> } | undefined;
    if (!combat) { const create = button("Create combat catalog"); create.onclick = () => { level.combat = { damageTypes: [{ id: "melee" }], profiles: [{ id: "player", faction: "player", maxHealth: 1, hurtboxes: [{ id: "body", x: 0, y: 0, width: 16, height: 16 }] }] }; commitProfileChange("Combat catalog created"); }; combatDetails.append(create); }
    else {
      const damage = document.createElement("input"); damage.value = combat.damageTypes.map((item) => item.id).join(", "); damage.setAttribute("aria-label", "Damage types"); damage.onchange = () => { combat.damageTypes = damage.value.split(",").map((id) => ({ id: id.trim() })).filter((item) => item.id); commitProfileChange("Damage types updated"); }; combatDetails.append(Object.assign(document.createElement("label"), { className: "property-field" })); combatDetails.lastElementChild!.append(Object.assign(document.createElement("span"), { textContent: "Damage types" }), damage);
      const states = Object.values((level.definitions ?? {}) as Record<string, { kind?: string; states?: Array<{ name?: string }> }>).filter((definition) => definition.kind === "character").flatMap((definition) => definition.states ?? []).map((state) => state.name).filter((name): name is string => Boolean(name));
      for (const [profileIndex, profile] of combat.profiles.entries()) { const card = document.createElement("div"); card.className = "gameplay-step"; const title = document.createElement("strong"); title.textContent = String(profile.id); card.append(title); const field = (label: string, key: string, type = "text"): void => { const control = document.createElement("input"); control.type = type; control.value = String(profile[key] ?? ""); control.onchange = () => { profile[key] = type === "number" ? Number(control.value) : control.value.trim(); commitProfileChange(`Combat ${label} updated`); }; const root = document.createElement("label"); root.className = "property-field"; root.append(Object.assign(document.createElement("span"), { textContent: label }), control); card.append(root); }; field("Profile ID", "id"); field("Faction", "faction"); field("Maximum health", "maxHealth", "number"); field("Invulnerability ticks", "invulnerabilityTicks", "number");
        for (const family of ["hurtboxes", "attacks", "guards"] as const) { const boxes = (profile[family] ??= []) as Array<Record<string, unknown>>; const heading = document.createElement("b"); heading.textContent = readableDataLabel(family); card.append(heading); boxes.forEach((box, boxIndex) => { const row = document.createElement("div"); row.className = "paired-actions"; for (const key of ["x", "y", "width", "height"] as const) { const control = document.createElement("input"); control.type = "number"; control.title = key; control.value = String(box[key]); control.onchange = () => { box[key] = Number(control.value); commitProfileChange(`Combat box ${key} updated`); }; row.append(control); } const state = document.createElement("select"); state.title = "Active state"; for (const value of states) state.append(Object.assign(document.createElement("option"), { value, textContent: value })); state.value = String((box.states as string[] | undefined)?.[0] ?? states[0] ?? ""); state.onchange = () => { box.states = [state.value]; commitProfileChange("Combat box state updated"); }; row.append(state); const remove = button("×", "Delete box"); remove.onclick = () => { boxes.splice(boxIndex, 1); commitProfileChange("Combat box deleted"); }; row.append(remove); card.append(row); }); const add = button(`+ ${readableDataLabel(family).slice(0, -1)}`); add.onclick = () => { const base: Record<string, unknown> = { id: `${family.slice(0, -1)}${boxes.length + 1}`, x: 0, y: 0, width: 16, height: 16 }; if (family !== "hurtboxes") base.states = [states[0] ?? "stop"]; if (family === "attacks") Object.assign(base, { damageType: combat.damageTypes[0]?.id ?? "melee", damage: 1 }); boxes.push(base); commitProfileChange("Combat box added"); }; card.append(add); }
        const remove = button("Delete profile"); remove.className = "danger-action"; remove.onclick = () => { combat.profiles.splice(profileIndex, 1); commitProfileChange("Combat profile deleted"); }; card.append(remove); combatDetails.append(card); }
      const addProfile = button("+ Combat profile"); addProfile.onclick = () => { combat.profiles.push({ id: `profile${combat.profiles.length + 1}`, faction: "neutral", maxHealth: 1, hurtboxes: [{ id: "body", x: 0, y: 0, width: 16, height: 16 }] }); commitProfileChange("Combat profile added"); }; combatDetails.append(addProfile);
    }
    inspector.append(combatDetails);
  }
  ...
  for (const [control, kind, label] of [[hurtboxes, "hurt", "Hurt · cyan"], [attackboxes, "attack", "Attack · red"], [guardboxes, "guard", "Guard · blue"]] as const) { control.setAttribute("aria-pressed", "true"); control.onclick = () => { const visible = control.getAttribute("aria-pressed") !== "true"; control.setAttribute("aria-pressed", String(visible)); control.textContent = `${label}: ${visible ? "on" : "off"}`; preview.setCombatBoxesVisible(kind, visible); }; }
