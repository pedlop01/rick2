...
export class PreviewRuntime {
  ...
  #visualFrame(definitionId: string, stateName: string, elapsedTicks: number): number {
    const definition = record(record(this.#source.definitions)[definitionId]), state = list(definition.states).map(record).find((candidate) => candidate.name === stateName), animation = record(state?.animation), duration = Math.max(1, number(animation.frameDurationTicks, 1)), count = list(animation.sprites).length;
    return count ? Math.floor(elapsedTicks / duration) % count : 0;
  }
