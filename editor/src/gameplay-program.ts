...
export function validateGameplayProgram(definition: GameplayProgramDefinition): void {
  ...
  const action = (value: GameplayAction): void => { if (value.type === "keepPlayerMoving") return; if (value.type === "forcePlayerState") { nonEmpty(value.state, "forcePlayerState.state"); nonEmpty(value.previousState, "forcePlayerState.previousState"); return; } if (value.type === "setPlayerMode") { if (typeof value.visible !== "boolean" || typeof value.controllable !== "boolean") throw new Error("invalid player mode action"); return; } if (value.type === "setEntityVisible") { if (value.entityType !== "backgroundObject" || !Number.isInteger(value.id) || value.id < 0 || typeof value.visible !== "boolean" || (value.restartAnimation !== undefined && typeof value.restartAnimation !== "boolean")) throw new Error("invalid entity visibility action"); return; } if (value.type === "emitEvent") { if (!knownEvent(value.event)) throw new Error(`event does not exist: ${value.event}`); return; } if (value.type === "setCamera") { if (!Number.isInteger(value.durationTicks) || value.durationTicks < 0 || (value.mode === "fixed" && ![value.x, value.y].every(Number.isFinite))) throw new Error("invalid camera action"); return; } if (value.type === "showMessage" || value.type === "hideMessage" || value.type === "playEffect") return; const flag = flags.get(value.flag); if (!flag) throw new Error(`flag does not exist: ${value.flag}`); if (value.type === "toggleFlag" && flag.type !== "boolean") throw new Error(`flag ${value.flag} must be boolean`); if (value.type === "incrementFlag" && (flag.type !== "number" || !Number.isFinite(value.amount))) throw new Error(`flag ${value.flag} must be a finite number`); if (value.type === "setFlag" && !valueMatches(flag.type, value.value)) throw new Error(`flag ${value.flag} value must be ${flag.type}`); };
...
export class GameplayState {
  ...
  execute(action: GameplayAction): void { if (action.type === "emitEvent") { this.#events.add(action.event); this.onEvent(action.event); return; } if (action.type === "setCamera" || action.type === "showMessage" || action.type === "hideMessage" || action.type === "playEffect" || action.type === "setEntityVisible" || action.type === "setPlayerMode" || action.type === "forcePlayerState" || action.type === "keepPlayerMoving") { this.onRegisteredAction(action); return; } const current = this.flag(action.flag); if (action.type === "setFlag") this.#flags.set(action.flag, action.value); else if (action.type === "toggleFlag") this.#flags.set(action.flag, !current); else this.#flags.set(action.flag, Number(current) + action.amount); }
