...
| 34 | Pulir la interfaz web para una primera versión human friendly | Alta | 31 | Completada | Los flujos de crear, abrir, editar, previsualizar, validar y guardar un nivel se entienden sin conocer el formato interno; se revisan jerarquía visual, nombres, formularios, selección, ayudas, feedback y adaptación a portátil sin reducir las capacidades avanzadas. |
| 35 | Dar soporte a proyectos con múltiples niveles | Alta | 31, 34 | Completada | El formato y el editor permiten crear, duplicar, renombrar, ordenar, abrir y eliminar niveles manteniendo un `initialLevel` válido; assets compartidos y referencias sobreviven a importación/exportación y cada nivel se valida por separado. |
| 36 | Definir campaña, progresión y restricciones entre niveles | Media | 35 | Completada | El proyecto declara un orden secuencial y reglas explícitas de desbloqueo; Rick puede recorrer los niveles principales en orden y el nivel final permanece bloqueado hasta completar los cuatro anteriores, sin codificar IDs concretos en el runtime. |
| 37 | Convertir el runtime C++ en un juego completo | Alta | 35, 36 | Completada | C++ dispone de intro, menú, selección o continuación de partida, transición entre niveles y estado de progreso; carga el nivel correspondiente desde la campaña y separa el shell de juego de la simulación. |
| 38 | Ejecutar en C++ los proyectos exportados por el editor | Alta | 35, 37 | Completada | El runtime abre o extrae de forma segura `.rick2-project`, resuelve manifiesto, nivel y assets, y ejecuta el mismo paquete validado en web; una prueba cubre exportación, reapertura y arranque nativo. |
| 39 | Formalizar Camelot Warriors como proyecto de validación | Alta | 34 | Completada | Se documenta una matriz de equivalencias entre ambos motores y se elige una escena vertical de Camelot con transformación, combate, script, cámara y presentación; cada carencia se clasifica como capacidad genérica, behavior del juego o conversión de datos, sin introducir excepciones por nombre de juego. |
| 40 | Incorporar máquinas de estados y formas de personaje declarativas | Alta | 31, 34, 39 | Completada | Un personaje puede declarar estados, transiciones, condiciones, acciones y perfiles o formas intercambiables sin modificar el núcleo; web y C++ ejecutan el mismo grafo validado y Rick conserva su comportamiento mediante una configuración equivalente. |
| 41 | Añadir flags, acciones y secuencias de gameplay tipadas | Alta | 39, 40 | Completada | Zonas y eventos evalúan condiciones sobre flags seguros y ejecutan acciones registradas; el catálogo de eventos admite declarar emisores y eventos personalizados, y un secuenciador combina pasos seriales y paralelos para mecanismos y cinemáticas, con validación, preview, undo/redo y sin punteros a variables ni JavaScript arbitrario. |
| 42 | Generalizar presentación, cámara y narrativa | Media | 34, 39 | Completada | El formato y ambos runtimes soportan parallax delantero y trasero, transiciones configurables entre vistas de cámara, diálogos o mensajes y un registro acotado de efectos visuales; el editor permite configurarlos y previsualizarlos. |
| 43 | Generalizar combate e IA para otros plataformas | Alta | 39-42 | Completada | Hitboxes de ataque, defensa, daño, transformaciones y behaviors enemigos como salto, vuelo, movimiento vertical y boss se componen mediante datos y registros compartidos; Rick y la escena vertical de Camelot se cubren con regresiones sin ramas específicas por juego. |
| 44 | Migrar Camelot Warriors al Rick2 Engine | Alta | 35, 39-43 | Completada | Las cuatro fases, assets, scripts, secuencias, textos, final histórico y postprocesado de agua funcionan en web y C++; la conversión es reproducible, no declara pérdidas y el proyecto antiguo solo permanece como referencia y fixtures en `tools/`. |
...
11. Tarea 39: fijar una escena vertical y una matriz de aceptación reales de Camelot.
12. Tareas 40-43: incorporar al núcleo las capacidades genéricas que esa escena exige.
13. Tareas 35-38: ampliar el producto a proyectos multinivel, campaña y juego C++ completo.
14. Tarea 44: migrar primero una fase y después el Camelot Warriors completo como prueba de fuego.
...
| 2026-08-24 | 33 | Auditado y reforzado el runtime C++ con las invariantes obtenidas del port web: IA determinista, escaleras, gravedad, bloques, bombas, acciones, animaciones, muerte, checkpoints y triggers; documentadas todas las divergencias confirmadas. | Build C++ limpio, contratos y fixtures nativos correctos, 126 pruebas web superadas y recorrido funcional validado manualmente por bloques. | Commit de paridad reforzada del runtime C++ |
| 2026-09-06 | 43 | Generalizados combate, perfiles por forma y behaviors enemigos; añadidos autoría y overlays web, resolución y overlays nativos, adaptador de IA v1 y fixture terrestre/aéreo sin ramas por juego. | Schema y conversión, build limpio mediante `bin/Makefile`, pruebas focales C++ y 18 suites web correctos mediante `tools/check_all.sh`; revisión aprobada. | Commit final de combate e IA |
| 2026-09-06 | 35 | Añadida gestión multinivel con creación, duplicado, selección, renombrado, orden, borrado, nivel inicial y redimensionado de mapas; preservadas rutas de assets, historial y validación por nivel. | Build y 19 suites web correctos; round-trip ZIP, assets compartidos, validación independiente y redimensionado cubiertos; flujo manual aprobado. | Commit de proyectos multinivel |
| 2026-09-21 | 44 | Completada y aceptada la migración funcional: cuatro fases, campaña, final histórico, textos exactos, control de enemigos, pendientes declarativas y efecto de agua web/C++; el informe de pérdidas queda vacío. | Conversión reproducible, revisión manual por bloques, suites web, regresiones nativas y build oficial limpio; commits previos `76d80a9`, `cc622c6`, `53b704c` y `e124e05`. | Commit final de cierre de Task 44 |
