...
void World::WorldStep(Character* player) {
  ...
  if (gameplay) {
    ...
    if (presentation) { presentation->Step(); gameplay->SetActionSink([this, player](const nlohmann::json& action) {
      if (action.value("type", "") == "setEnemyActive") { const int id = action.at("id").get<int>(); Enemy* target = NULL; for (vector<Character*>::iterator it = enemies.begin(); it != enemies.end(); ++it) { Enemy* candidate = static_cast<Enemy*>(*it); if (candidate->GetId() == id) { target = candidate; break; } } if (!target) throw DataLoadError("Enemy does not exist: " + std::to_string(id)); if (action.value("reset", false)) target->Reset(); target->SetActive(action.at("active").get<bool>()); return; }
      if (action.value("type", "") == "keepPlayerMoving") { Player* scene_player = dynamic_cast<Player*>(player); if (scene_player) scene_player->KeepMoving(); return; }
      if (action.value("type", "") == "forcePlayerState") { Player* scene_player = dynamic_cast<Player*>(player); if (scene_player) scene_player->ForceState(action.at("state").get<std::string>(), action.at("previousState").get<std::string>()); return; }
      if (action.value("type", "") == "setPlayerMode") { Player* scene_player = dynamic_cast<Player*>(player); if (scene_player) scene_player->SetSceneMode(action.at("visible").get<bool>(), action.at("controllable").get<bool>()); return; }
      if (action.value("type", "") == "setEntityVisible") { const int id = action.at("id").get<int>(); Object* object = FindByTypeId(back_objects, id); if (!object) throw DataLoadError("Background object does not exist: " + std::to_string(id)); object->SetVisible(action.at("visible").get<bool>()); object->SetActive(action.at("visible").get<bool>()); if (action.value("restartAnimation", false)) static_cast<StaticObject*>(object)->RestartAnimation(); return; }
      const double follow_x = player->GetCorrectedPosX() - GetCameraConfig().width / 2.0, follow_y = player->GetCorrectedPosY() - GetCameraConfig().height / 2.0; presentation->Execute(action, presentation->CameraX(follow_x), presentation->CameraY(follow_y));
    }); }
