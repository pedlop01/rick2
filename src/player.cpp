...
void Player::ApplyFormProfile(const nlohmann::json& form) {
  ...
  action_up_state = binding(bindings, "up", action_up_state); action_down_state = binding(bindings, "down", action_down_state); action_horizontal_state = binding(bindings, "horizontal", action_horizontal_state);
...
void Player::LoadFormAnimations(const std::string& definition_id) {
  ...
  try {
    for (nlohmann::json::const_iterator state_value = definition.at("states").begin(); state_value != definition.at("states").end(); ++state_value) {
      ...
      Animation* value = new Animation(bitmap, animation.at("frameDurationTicks").get<unsigned int>());
...
void Player::ComputeNextState(World* map, Keyboard& keyboard) {
  ...
  CharacterStateContext context; context.x = pos_x; context.y = pos_y;
  context.controls["left"] = keyboard.PressedLeft(); context.controls["right"] = keyboard.PressedRight(); context.controls["up"] = keyboard.PressedUp(); context.controls["down"] = keyboard.PressedDown(); context.controls["action"] = keyboard.PressedSpace();
  context.signals["grounded"] = !inAir; context.signals["onStairs"] = inStairs; context.signals["canDescendStairs"] = overStairs; context.signals["canStand"] = !collisionHeadOrig; context.signals["ceilingBlocked"] = collisionHeadOrig;
  const int requested = GroundActionState(keyboard); if (requested == CHAR_STATE_SHOOTING) context.actions.insert("shooting"); else if (requested == CHAR_STATE_BOMBING) context.actions.insert("bombing"); else if (requested == CHAR_STATE_HITTING) context.actions.insert("hitting");
  ...
  const std::string previous_form = forms->ActiveForm(); forms->Step(context); if (forms->ActiveForm() != previous_form) ApplyFormProfile(forms->ActiveDefinition());
  state = BehaviorState(ActiveState(*forms));
  SelectFormAnimation();
  if (state == CHAR_STATE_RUNNING) direction = keyboard.PressedLeft() ? CHAR_DIR_LEFT : keyboard.PressedRight() ? CHAR_DIR_RIGHT : CHAR_DIR_STOP;
  else if (state == CHAR_STATE_STOP || state == CHAR_STATE_CROUCHING) { direction = CHAR_DIR_STOP; FixHorizontalDirection(keyboard); }
  else if (state == CHAR_STATE_CLIMBING) direction = keyboard.PressedUp() ? CHAR_DIR_UP : keyboard.PressedDown() ? CHAR_DIR_DOWN : CHAR_DIR_STOP;
  else if (state == CHAR_STATE_JUMPING) { if (old_state != CHAR_STATE_JUMPING) { direction = context.signals["grounded"] && keyboard.PressedUp() ? CHAR_DIR_UP : CHAR_DIR_DOWN; pos_y_chk = pos_y; } else if ((direction & CHAR_DIR_UP) && (collisionHead || abs(pos_y_chk - pos_y) >= jump_height)) direction = CHAR_DIR_DOWN; FixHorizontalDirection(keyboard); }
  ...
  else if (state == CHAR_STATE_BOMBING) { direction = keyboard.PressedLeft() ? CHAR_DIR_LEFT : keyboard.PressedRight() ? CHAR_DIR_RIGHT : CHAR_DIR_STOP; const bool created = map->CreateNewBomb(pos_x, pos_y, direction == CHAR_DIR_LEFT ? OBJ_DIR_LEFT : direction == CHAR_DIR_RIGHT ? OBJ_DIR_RIGHT : OBJ_DIR_STOP); if (created && GetRuntimeAudioBindings().bomb >= 0) sound_handler->PlaySound(GetRuntimeAudioBindings().bomb, false); }
  else if (state == CHAR_STATE_HITTING) direction = keyboard.PressedLeft() ? CHAR_DIR_LEFT : keyboard.PressedRight() ? CHAR_DIR_RIGHT : face;
